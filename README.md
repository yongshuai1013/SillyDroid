...
- 设置页导入会自动识别上游 Tavern 用户备份 ZIP，以及两种完整数据 ZIP：Docker 四目录结构（`config`、`data`、`plugins`、`extensions`）和 Linux / Termux public 结构（`config`、`data`、`plugins`、`public`）。
- 从原版酒馆迁移时，可导入官方用户备份，把主要用户数据迁入当前 Android 宿主。
- 如果手头是 Docker 风格完整数据包，则会按 `config`、`data`、`plugins`、`extensions` 四目录恢复；如果手头是 Linux / Termux 风格完整数据包，则要求根目录为 `config`、`data`、`plugins`、`public`，并把 `public/scripts/extensions/third-party` 识别为第三方扩展目录导入。
...
- 这个脚本会导出 Linux / Termux public 结构 ZIP：根目录为 `config`、`data`、`plugins`、`public`，其中第三方扩展位于 `public/scripts/extensions/third-party`；不会把程序本体、`node_modules`、源码或其他运行时文件打进 ZIP。
- 默认会自动探测官方 Termux 安装目录（通常是 `~/SillyTavern`），并优先选择看起来确实有用户数据的根目录；如果机器上同时存在源码目录和实际运行目录，建议显式传 `--install-root`。
